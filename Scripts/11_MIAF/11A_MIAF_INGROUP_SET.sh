...
#
# This script runs MIAF on the INGROUP orthogroups.
#
# Method used here:
#   MAFFT + IQ-TREE
#
...
# OSG INGROUP output folder containing the FASTA files
INPUT=##                        # Change this

# MIAF INGROUP output folder
OUTPUT=##                       # Change this
...
if [ ! -d "${INPUT}" ]; then
    echo "ERROR: INGROUP input folder not found:"
    echo "${INPUT}"
    exit 1
fi
...
"${MIAF_PATH}" \
    ...
    || {
        echo
        echo "ERROR: MIAF INGROUP failed."
        exit 1
    }
...
echo
echo "============================================================"
echo "MIAF INGROUP completed successfully"
echo "============================================================"
